export declare const AGENT_GLOW_STATES: readonly ["idle", "listening", "thinking", "speaking", "tool-running", "waiting", "blocked", "error", "success", "interrupted"];
...
export type AgentGlowEvent = {
    type: 'presence.state.set';
    state: AgentGlowState;
    meta?: AgentGlowEventMeta;
} | {
    type: 'presence.turn.started';
    source?: 'user' | 'agent' | 'system';
    runId?: string;
    message?: string;
} | {
    type: 'presence.turn.completed';
    runId?: string;
    message?: string;
} | {
    ...
    type: 'presence.audio.input';
} & AgentGlowInput) | ({
    type: 'presence.audio.output';
} & AgentGlowInput) | {
    ...
    type: 'presence.tool.completed';
    toolName?: string;
    runId?: string;
    message?: string;
} | {
    type: 'presence.tool.failed';
    toolName?: string;
    runId?: string;
    recoverable?: boolean;
    message?: string;
    ...
    reason?: AgentGlowWaitReason | string;
    runId?: string;
    message?: string;
} | {
    type: 'presence.blocked';
    reason?: AgentGlowBlockedReason | string;
    runId?: string;
    message?: string;
} | {
    type: 'presence.error';
...
export interface AgentGlowSnapshot {
    ...
    input: Required<Pick<AgentGlowInput, 'speechLevel' | 'inputLevel' | 'activityLevel' | 'progress'>> & {
