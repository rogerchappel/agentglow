...
import { AGENT_GLOW_STATES, } from "./types.js";
const STATE_META = {
...
function cleanMeta(meta = {}) {
    ...
    for (const [key, value] of Object.entries(meta)) {
        if (value === undefined || typeof value === 'function' || typeof value === 'symbol')
            continue;
        if (key === 'progress')
            out.progress = clamp(value);
        else if (typeof value === 'string' || typeof value === 'number' || typeof value === 'boolean')
            out[key] = value;
    }
    return out;
}
function normalizeInput(input = {}, previous, smoothing = 0.35) {
    const blend = (next, prior, fallback = 0) => {
        if (next === undefined)
...
export function mapAgentGlowEventToState(event) {
    switch (event.type) {
        ...
        default: throw new Error(`Unsupported AgentGlow event: ${event.type}`);
    }
}
export function getAgentGlowStateMeta(state) {
    if (!AGENT_GLOW_STATES.includes(state))
        throw new Error(`Unknown AgentGlow state: ${String(state)}`);
    return STATE_META[state];
}
export function normalizeAgentGlowTheme(theme = {}) {
...
export function createAgentGlowCssVars(theme = {}, prefix = '--agentglow') {
...
export function createAgentGlowController(options = {}) {
    let destroyed = false;
    ...
    return {
        ...
        send(event) {
            switch (event.type) {
                case 'presence.state.set':
                    setState(event.state, event.meta);
                    break;
                ...
                case 'presence.audio.input':
                    input = normalizeInput({ inputLevel: event.inputLevel, frequencyBands: event.frequencyBands }, input, options.smoothing);
                    emit();
                    break;
                case 'presence.audio.output':
                    input = normalizeInput({ speechLevel: event.speechLevel, frequencyBands: event.frequencyBands }, input, options.smoothing);
                    emit();
                    break;
        ...
        setInput(nextInput) { input = normalizeInput(nextInput, input, options.smoothing); meta = { ...meta, ...(nextInput.progress !== undefined ? { progress: input.progress } : {}) }; emit(); },
        setTheme(nextTheme) { theme = normalizeAgentGlowTheme({ ...theme, ...nextTheme, palette: { ...theme.palette, ...nextTheme.palette }, motion: { ...theme.motion, ...nextTheme.motion } }); emit(); },
        getSnapshot: snapshot,
        subscribe(listener) { listeners.add(listener); listener(snapshot()); return () => { listeners.delete(listener); }; },
        destroy() { destroyed = true; listeners.clear(); },
    };
}
export function createAgentGlowAudioSmoother(options = {}) {
    ...
    return {
        push(samples) {
            const rawLevel = 'speechLevel' in samples || 'inputLevel' in samples
                ? clamp(samples.speechLevel ?? samples.inputLevel)
                : Array.from(samples).reduce((sum, sample) => sum + Math.abs(Number(sample) || 0), 0) / Math.max(1, samples.length);
            level = level + (clamp(rawLevel) - level) * (rawLevel > level ? attack : release);
            silentFrames = level < silenceThreshold ? silentFrames + 1 : 0;
            const bands = 'frequencyBands' in samples && samples.frequencyBands ? Array.from(samples.frequencyBands, clamp) : [level, Math.min(1, level * 0.75), Math.min(1, level * 0.5)];
            return { level: Number(level.toFixed(4)), bands, silent: silentFrames >= requiredSilentFrames };
        },
        reset() { level = 0; silentFrames = 0; },
    };
}
export function createAgentGlowMockAnalyser(frames = []) {
    let index = 0;
    return {
        next() {
...
export function createAgentGlowTimeline(events, options = {}) {
...
export function readAgentGlowAnalyserFrame(analyser, options = {}) {
...
export async function createMicrophoneAgentGlowInput() {
    ...
    return { stream, permission: 'granted' };
}
function esc(value) { return value.replace(/[&<>\"]/g, (ch) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;' })[ch]); }
export function renderAgentGlowToSvg(snapshot, options = {}) {
    ...
    const warnings = reduced ? [{ code: snapshot.reducedMotion ? 'reduced-motion' : 'low-power-mode', message: 'Renderer is using a static low-motion pose.' }] : [];
    ...
    let body = '';
    if (theme.shape === 'orb')
        body = `<g transform="translate(${cx} ${cy})"><circle r="${size * (0.2 + energy * 0.06)}" fill="url(#g)" filter="url(#glow)">${pulse}</circle><circle r="${size * 0.31}" fill="none" stroke="${accent}" stroke-width="2" stroke-opacity="0.45" stroke-dasharray="8 12"/></g>`;
    if (theme.shape === 'waveform-halo')
        body = `<g fill="none" stroke="${accent}" stroke-linecap="round">${Array.from({ length: 28 }, (_, i) => { const a = (i / 28) * Math.PI * 2; const amp = 18 + energy * 36 * ((i % 5) / 5 + 0.35); const x1 = cx + Math.cos(a) * size * 0.22; const y1 = cy + Math.sin(a) * size * 0.22; const x2 = cx + Math.cos(a) * (size * 0.22 + amp); const y2 = cy + Math.sin(a) * (size * 0.22 + amp); return `<path d="M${x1.toFixed(1)} ${y1.toFixed(1)}L${x2.toFixed(1)} ${y2.toFixed(1)}" stroke-width="${1 + energy * 4}" opacity="${0.35 + energy * 0.45}"/>`; }).join('')}</g>`;
    if (theme.shape === 'constellation')
        body = `<g>${Array.from({ length: 22 }, (_, i) => { const x = 40 + ((i * 53) % (width - 80)); const y = 40 + ((i * 89) % (height - 80)); return `<circle cx="${x}" cy="${y}" r="${2 + ((i % 4) * energy)}" fill="${i % 3 ? accent : p.accentSecondary}" opacity="${0.35 + energy * 0.55}"/>`; }).join('')}<circle cx="${cx}" cy="${cy}" r="${size * 0.18}" fill="none" stroke="${accent}" opacity="0.7"/></g>`;
    if (theme.shape === 'console-pulse')
        body = `<g>${Array.from({ length: 9 }, (_, i) => `<rect x="${44 + i * 26}" y="${cy - 12 - (i % 3) * energy * 18}" width="14" height="${24 + energy * 42 * ((i % 4 + 1) / 4)}" rx="4" fill="${accent}" opacity="${0.35 + i / 14}"/>`).join('')}<path d="M46 ${cy + size * 0.22} H ${width - 46}" stroke="${p.muted}" stroke-width="2" stroke-dasharray="4 8"/></g>`;
    if (theme.shape === 'minimal-dot-field')
        body = `<g>${Array.from({ length: 36 }, (_, i) => `<circle cx="${28 + ((i * 41) % (width - 56))}" cy="${28 + ((i * 67) % (height - 56))}" r="${1.5 + energy * (i % 5)}" fill="${i % 6 === 0 ? accent : p.muted}" opacity="${0.25 + energy * 0.5}"/>`).join('')}</g>`;
...
export function renderPresetFixture(preset, state = 'thinking') {
